...
- 一次输入动作合并：管道输入整批内容作为一次处理，交互式输入按缓冲区连片零等待合并（无时间窗口）；
...
左侧大纲只显示简短预览：每行由 `mN · 正文预览` 组成，预览把多行正文压成单行并截断；信任级别不写文字，而以低饱和绿（可信/当前用户指令）与低饱和红（不可信数据）整行提示，无颜色模式下不附加文字标签，选中条目后可在详情标题查看 `信任=` 与 `状态=`。空白/空内容片段永远不进大纲；`untrusted_data` 片段默认折叠，按 `W` 切换显示。图例同时解释 `included/excluded/empty/unknown_source`、`trusted_instruction/trusted_metadata/user_instruction/untrusted_data` 与 `message:N`；来源详情明确分列 `来源数`、`类型`、`路径`、`source_id`、`producer` 与 `entity_ids`，其中 `entity_ids` 是 `rec-*`、`turn-*` 等实体 UUID，不代表聊天顺序。详情视图默认隐藏来源明细，按 `W` 展开后显示全部来源明细；`C` 复制始终包含完整审计块。该折叠与配色只改变 TUI renderable，不改变 provider payload、RequestPart、span 或 token 估算；80×24 终端仍可滚动查看完整 trace 并操作按钮。
