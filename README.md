...
TUI 的来源详情明确分列 `来源数`、`类型`、`路径`、`source_id`、`producer` 与 `entity_ids`；其中 `entity_ids` 是 `rec-*`、`turn-*` 等实体 UUID，不代表聊天顺序。图例同时解释 `included/excluded/empty/unknown_source`、`trusted_instruction/trusted_metadata/user_instruction/untrusted_data` 与 `message:N`。折叠模式默认隐藏所有来源明细，并将只含换行/空格的 included part 连同其标题整段隐藏；片段标题仍保留 `来源数`。按 `W` 后以转义文本展开空白片段和全部来源明细，`C` 复制也始终包含完整审计块。该折叠与配色只改变 TUI renderable，不改变 provider payload、RequestPart、span 或 token 估算；80×24 终端仍可滚动查看完整 trace 并操作按钮。
