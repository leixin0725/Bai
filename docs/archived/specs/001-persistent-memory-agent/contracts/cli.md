...
3. 从 stdin 读取用户输入，经运行时外壳的统一串行消息管道处理；EOF 正常退出，Ctrl+C 第一次优雅停止（当前轮完成后退出）、第二次立即中止（2026-08-08）。
4. 每条输入依照 SingleTurnController 契约处理；同一时刻只处理一个对话动作，处理中到达的输入进入等待队列，不并发处理同一会话（2026-08-08）。

停止语义（2026-08-08）：SIGINT 退出码为 130；SIGTERM 优雅停止后返回 0；停止后不再接收新输入，当前轮按既有事务/pending 语义结束或中止。
