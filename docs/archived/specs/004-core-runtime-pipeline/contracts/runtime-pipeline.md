...
- 交互式 TTY（2026-08-10）：使用 raw 模式行编辑器——**Enter 发送、Shift+Enter 换行**（通过 Kitty 键盘协议识别：请求标志 1|4（`CSI >1;4u`，消歧义 + 备用键）后终端把 Shift+Enter 编码为 `CSI 13;2u`；不支持的终端回退为发送并可用 Ctrl+J 换行；同时兼容 VS Code `sendSequence` 与 codex 生态常见的 `ESC+CR` 传统注入编码）、退格删除、Ctrl+D 空缓冲为 EOF/有内容为提交；方向键与历史暂不支持。
- 括号粘贴（2026-08-10）：编辑器启用 `CSI ?2004h`，一次粘贴的 `CSI 200~ … 201~` 内容整体插入缓冲并正常回显，**标记不回显、不进入动作正文**；粘贴不自动发送，按 Enter 提交。退出与调试 TUI 结束（`resume()`）时确保关闭/重新启用，终端不残留 raw/2004/kitty 状态。
- 单次动作内容不截断、不按行拆轮；逐行输入（缓冲区空）每行独立成动作。
- 非 TTY/不支持编辑器的路径（管道）回退到逐行输入源与"内部缓冲/`select` 非空"的尽力合并；仍不做时间窗口。
- TUI 等独占终端的组件运行期间，输入源可被暂停：暂停即移除 fd 监听，独占方（Textual 驱动）可安全接管同一 stdin；恢复后重新监听并继续原合并语义，已缓冲字节不丢失、不误报 EOF。
- 事件循环若无法对 stdin 做非阻塞判定，文档化降级为逐行处理；Ubuntu/WSL 主平台必须实现合并。
