...
验收时固定检查：密集消息只有首 marker；相邻 gap 恰好 30 分钟时新分段；连续短间隔在最近 marker 后恰好 120 分钟时刷新；跨 `Asia/Shanghai` 自然日和时间倒退时分段；同刻事件保持一段。输出使用完整日期、分钟和 offset。`current_input` 使用本轮 provisional USER record 的既定 `created_at`，其可信 marker 位于可见 untrusted boundary 外、用户正文位于边界内并保持 user instruction；人格及状态指令保持无标记。原始 `role: content` 是历史 marker 后的逐字连续子串，历史 marker 仍在历史边界内。
