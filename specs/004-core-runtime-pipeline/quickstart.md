...
交互式终端（含 `--debug-prompts` 模式，2026-08-10）：raw 模式行编辑器——**Enter 发送、Shift+Enter 换行**（终端不支持 Shift+Enter 时可用 Ctrl+J）、退格删除；一次性粘贴多行时，括号粘贴内容整体进入编辑缓冲并正常回显，**终端不显示 `200~`/`201~` 标记**，按 Enter 后作为一条消息提交；逐行手动输入行为不变。
