...
**Git Milestones**: 四个可独立验证的原子提交（与 `tasks.md` 检查点对齐，每阶段实现、测试与受影响文档同提交）：

1. **管道与生命周期**：新增串行处理管道与运行时外壳，`chat` 切换到持久事件循环；优雅停止、pending 语义与退出码保持既有契约；BR-001/002/005 测试 + README/quickstart/CLI 契约同步。
2. **输入合并与状态查看**：stdin 输入读取器（管道 EOF 整批、终端缓冲连片合并、零等待）；`:status` 会话内状态命令；BR-006/007 测试 + 契约同步。
3. **最小后台执行器与事件投递**：后台执行器（提交/串行/状态）；定时事件、系统事件进入管道的投递入口与测试处理钩子；BR-004 测试 + 契约同步。
4. **配置分组与验收收尾**：配置分组表与 `config validate` 分组输出、重载状态接入 `:status`；BR-003 测试（含重载失败时终端出现明确警告且 `:status` 一致）；全部文档（README、quickstart、三份契约、迁移计划）一致性校验与全量回归。
