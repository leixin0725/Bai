...
- 括号粘贴（2026-08-10）：POSIX TTY 启动时启用 `CSI ?2004h`，终端会把一次粘贴包裹在 `CSI 200~ … 201~` 之间；读取器剥离标记，在 `200~` 与 `201~` 之间持续累积，收到 `201~` 才提交整批为一次动作。这样即使终端把粘贴内容逐行延迟送达，也不会在第一个回车处误判为发送；标记不进入动作正文。退出与调试 TUI 结束（`resume()`）时确保重新启用/关闭，终端不残留 2004 状态。
- 单次动作内容不截断、不按行拆轮；逐行输入（缓冲区空）每行独立成动作。
- 不支持括号粘贴的终端回退到"内部缓冲/`select` 非空"的尽力合并；仍不做时间窗口。
